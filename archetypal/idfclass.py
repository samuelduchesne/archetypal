...
class IDF(geomeppy.IDF):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.eplus_run_options = EnergyPlusOptions(
            self.idfname, None, ep_version="-".join(map(str, self.idd_version))
        )
...
def load_idf(eplus_file, idd_filename=None, output_folder=None, include=None):
    ...
    *archetypal.settings.use_cache* is true, then the idf object is loaded
    ...
    if idf:
        ...
        idf = eppy_load(
            eplus_file, idd_filename, output_folder=output_folder, include=include
        )
...
def prepare_outputs(
    eplus_file, outputs=None, idd_filename=None, output_folder=None, save=True
):
    ...
    idf = load_idf(eplus_file, idd_filename=idd_filename, output_folder=output_folder)
                ...
                results.extend(
                    [
                        (
                            file.basename(),
                            process(
                                file, working_dir=os.getcwd(), simulname=output_prefix
                            ),
                        )
