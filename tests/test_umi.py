...
def test_umi_routine(config):
    ...
    a = UmiTemplate.from_idf(
        idf, wf, load=True, run_eplus_kwargs=dict(prep_outputs=True), name="Mixed_Files"
    )
...
def test_umi_samples(config):
    ...
    a = UmiTemplate.from_idf(
        ...
        wf,
        load=True,
        run_eplus_kwargs=dict(prep_outputs=True, expandobjects=True),
        name="Umi_Samples",
    )
    print(a.BuildingTemplates)
